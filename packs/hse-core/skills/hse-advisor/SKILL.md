...
| **Geothermal energy** sector (strongly NZ — Taupō Volcanic Zone) — geothermal fields/steamfields, wells, generation, direct use; the signature hazards H₂S (toxic, rapid knockdown) and high-temperature/high-pressure steam & fluid, well discharge/blowout, hydrothermal eruptions/hot ground, silica scaling; generation/network → electricity-supply, drilling → drilling | **geothermal-specialist** |
| **Drilling rig & rotary-drilling operation** (cross-sector — oil & gas, geothermal, water-bore, mineral exploration) — the rig/derrick, mast rig-up/down, rotating drill-string entanglement, making/breaking connections, hoisting, the mud/circulating system, generic well-control awareness; the sector regime routes to oil-gas / geothermal / mining | **drilling-specialist** |
...
| **Waste & recycling** sector — kerbside collection, transfer stations, MRFs/sorting, landfills, composting, hazardous/clinical waste; a high-fatality sector — struck-by mobile plant/vehicles, machinery crush (compactors/balers/shredders), lithium-ion battery fires, unknown/mixed-waste chemical & biological hazards; routes each hazard method to its SME | **waste-recycling-specialist** |
| **Oil & gas / petroleum** sector — upstream/offshore exploration, drilling, wells, production, gas processing; major-accident-hazard sector on a safety-case regime — loss of containment/fire-explosion, well control/blowout (BOP), H2S/sour gas, offshore escape/evacuation/helicopter/SIMOPS; the regime split (NZ WorkSafe High Hazards Unit / AU NOPSEMA offshore vs state onshore); general process-safety/MHF → manufacturing | **oil-gas-specialist** |
| **Food, meat & dairy processing** sector (WORKER safety, not food safety) — abattoirs/meatworks, boning, rendering, dairy/milk-powder, seafood; knives & cut-resistant PPE, processing machinery (bandsaws/mincers), ammonia refrigeration (toxic release/MHF), wet-floor slips, cold, repetitive overuse (OOS), zoonoses (leptospirosis NZ / Q fever AU); general factory/MHF/dust → manufacturing | **food-processing-specialist** |
| **Rail** sector — heavy/metro/light rail & rail freight; the dedicated rail-safety regime co-regulated alongside WHS/HSWA (AU ONRSR / Rail Safety National Law; NZ Waka Kotahi/NZTA, Railways Act 2005; ATSB/TAIC no-blame investigators); track-worker protection, level crossings, shunting/yards, overhead-line/traction, SPAD, rail-safety-worker fatigue/D&A | **rail-specialist** |
| **Hospitality** sector — restaurants, cafés, bars/pubs/clubs, hotels/motels/accommodation, catering, fast food; young/casual/high-turnover workforce — burns/hot oil, knives/cuts, wet-floor slips, manual handling/housekeeping; plus the licensed-venue violence/robbery & psychosocial/sexual-harassment tail (routed to the violence & psychosocial SMEs) | **hospitality-specialist** |
...
above. `hse-core` (this pack) always has the orchestrator and the cross-cutting specialists, and
every other pack installs it automatically; the rest are optional:
...
its pack is not installed — and each pack installs into its own folder, so its files are not on
disk either:

1. **Tell the user once** which pack carries it and how to add it — e.g. "`forestry-specialist` is
   in the `hse-sector-primary` pack: `/plugin install hse-sector-primary@hse-nz-au-skills`".
2. **Carry on with what is installed** — the core specialists (duties, critical risk, task
   analysis, SOPs) and general good practice — and say plainly that the specialist method was not
   applied.
3. **Do not invent the missing specialist's content**, and do not go looking for its files.

**Reading the specialists' cross-references.** A hand-off names the skill (`crane-specialist`). A
pointer written like `worksafe-nz-specialist:references/overlapping-duties.md` means *that file
inside that skill* — invoke the skill to reach it. It is written that way because the skill may
live in a different pack.
